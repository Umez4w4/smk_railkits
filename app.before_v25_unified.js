(function(){
'use strict';
const FRONTEND_VERSION='2026-09-18-v24-final',C=window.RailCore,$=id=>document.getElementById(id);
...
// RAIL_LOG_V21_SECTION_DISPLAY_FIX
// Keep eligibility strict for statistics and selection, but do not create
// visual holes when an otherwise-ineligible piece is part of an actual
// station-to-station section.
let mapSectionDisplayPieceIds=new Set();

function rebuildMapSectionDisplayPieceIds(){
  mapSectionDisplayPieceIds=new Set(
    (model?.sections||[])
      .flatMap(s=>s?.piece_ids||[])
      .map(String)
      .filter(Boolean)
  );
}

// RAIL_LOG_V23_FINAL_VISIBILITY
function featureVisibleOnMap(f){
  if(featureEligibleForMap(f))return true;
  const p=f?.properties||{};
  return p.display_only===true||p.display_only==='true'||p.section_referenced===true||p.section_referenced==='true';
}
...
function railLineFeatureCollection(){
  ...
  for(const f of model.geo.features||[]){
    if(!featureVisibleOnMap(f))continue;
