...
// RAIL_LOG_V27_RECORD_DATE_EDITOR
...
function updateRecordDate(id,value){
  ...
  if(!trip){
    status(
      zh?'找不到要编辑的乘车记录。':'編集する乗車記録が見つかりません。',
      'error'
    );
    return;
  }
  ...
  status(
    v
      ?(zh?`乘车日期已更新为 ${v}。`:`乗車日を ${v} に更新しました。`)
      :(zh?'已清除乘车日期。':'乗車日の指定を解除しました。'),
    'ok'
  );
}
...
function renderHistory(){
  ...
  els.history.innerHTML=[...trips].reverse().map(t=>{
    const names=t.station_names||[],
          ...
          dateValue=editableTripDateValue(t),
          id=C.esc(t.id||'');
...
function bind(){
  ...
  els.statsView.addEventListener('change',renderStats);els.history.addEventListener('change',ev=>{const input=ev.target?.closest?.('.history-date-input');if(input)updateRecordDate(input.dataset.tripId,input.value)});els.selectAllRecords.addEventListener('click',toggleSelectAll);els.deleteSelected.addEventListener('click',batchDelete);els.undo.addEventListener('click',undo);els.export.addEventListener('click',exportBackup);els.import.addEventListener('click',()=>els.importFile.click());els.importFile.addEventListener('change',()=>{if(els.importFile.files[0])importBackup(els.importFile.files[0]);els.importFile.value=''})
}
